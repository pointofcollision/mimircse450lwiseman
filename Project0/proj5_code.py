...
#todo:make this return char literals in a list then use the
...
class ArrayValNode(BaseNode):
    ...
    def generate_bad_code(self,scope):
        ...
        for i in range(0,len(self.children[0])):
            #get new ID
            new_entry = get_entry();
            #for this new id, define its parent, give it the array_type, and index as well
            child_entry = self.children[0][i].generate_bad_code(scope)
            #(self,type_, value,scope = 1, active = True, var_name= None, size = None,array_type = False,array_index=None,array_parent = None):
            bad_code_generated.append("ar_set_idx {} {} {}".format(entry,i, child_entry))
        #children will consist of a sequence of ValLiteralNode's
...
class BinaryComparisonNode(BaseNode):
    ...
    def generate_bad_code(self, scope):
        ...
        if (self.operator == "=="):
            ...
            print("comparing ==")
            # the actual value storing is only useful for printing out stuff
            #does not affect bad code we just need the types
            #result = int(generated_global_variables[child_0_entry].value.children[-1] == generated_global_variables[child_1_entry].value.children[-1])
            #print(result)
...
class BinaryMathNode(BaseNode):
    ...
    def generate_bad_code(self, scope):
        ...
        if (self.operator == '*'):
            ...
            #result = generated_global_variables[child_0_entry].value.children[-1] * generated_global_variables[child_1_entry].value.children[-1]
            #print(result)
...
class IdAssignmentNode(BaseNode):
    ...
    def generate_bad_code(self,scope):
        ...
        if(entry_table.array_index ==None):
            ...
            index_register = entry_table.array_index.generate_bad_code(scope)
...
def p_expr_cmpd_sub(p):
    ...
    value = p[3]
    #GLOBAL_VARIABLES[p[1].ID][0] = BinaryMathNode([p[1],p[3]],"-")
    #p[0] = IdAssignmentNode([IdAccessNode([GLOBAL_VARIABLES[p[1].ID][0]],p[1].ID),BinaryMathNode([IdAccessNode([GLOBAL_VARIABLES[p[1].ID][0]],p[1].ID),p[3]],"-")])
    p[0] = IdAssignmentNode([p[1],BinaryMathNode([p[1],p[3]],"-")])
...
input_ = r"""
# Make sure size and resize can still be used as variables.
