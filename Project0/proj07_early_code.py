...
#the return type, name, and number of parameters.
class FunctionEntry():
    def __init__(self, name, return_type_, num_params, parameter_types_ = []):
        ...
        self.param_types = parameter_types_
...
class FunctionNode(BaseNode):
    ...
    def generate_bad_code(self, scope, bad_code_generated):
        ...
        for i in range(0, len(self.children) - 1):
            ...
            param_entry = self.children[i].generate_bad_code(scope + 1, function_code_generated_global)
            param_type = generated_global_variables[param_entry].type_
            print("parameter type : " + param_type)
            #update type in FunctionEntry 
            global_functions[self.ID].param_types.append(param_type) 
        #execute function block 
        self.children[-1].generate_bad_code(scope, function_code_generated_global)
        function_scope = 0;
...
class ReturnNode(BaseNode):
    ...
    def generate_bad_code(self, scope, bad_code_generated):
        ...
        #TODO: return node should use the stack to know where to return to
...
def p_function_define(p):
    ...
    if (p[3] in global_functions):
        raise NameError("ERROR("+ str(size_of_input-statement_count) +"): function already exists '"+ str(p[3]) +"' ")
    if (len(p) == 7):
        def_funct = FunctionNode(p[5],p[2],p[3])
    #FunctionEntry( name, return_type_, parameter_types_, num_params)
        new_entry = FunctionEntry(p[3], p[2], len(p[5]))
        global_functions[p[3]] = new_entry
        p[0] = def_funct
    else:
        def_funct = FunctionNode([],p[2],p[3])
        new_entry = FunctionEntry(p[3], p[2], 0)
        global_functions[p[3]] = new_entry
        p[0] = def_funct
...
def p_function_params(p):
    """
    function_params : function_params ',' declaration
                    | declaration
    """
    arguments_array = []
    length = len(p)
...
define array(char) echo(array(char) x) {
  if (x[4] == 'h') print("77"); 
  return x;
}
val a = 4;
